//===--Proj526Unroll.h - Loop unrolling utilities -------------------------===//
//
...
bool llvm::UnrollLoop(Loop *L, unsigned Count, unsigned TripCount, bool Force,
  ...
  for (unsigned It = 1; It != Count; ++It) {
    ...
    for (LoopBlocksDFS::RPOIterator BB = BlockBegin; BB != BlockEnd; ++BB) {
      ValueToValueMapTy VMap;
      char *suffix;
      sprintf(suffix, "__526_%d", It);
      //BasicBlock *New = CloneBasicBlock(*BB, VMap, "." + Twine(It));
      BasicBlock *New = CloneBasicBlock(*BB, VMap, Twine(suffix));
  ...
  for (unsigned i = 0, e = Latches.size(); i != e; ++i) {
    ...
    if (CompletelyUnroll) {
      ...
      if (ItersToPreserve.size() > 0) {
        NeedConditional = false;
        // Need conditional is only true if this is not the last iteration,
        // and the iteration appears in ItersToPreserve
        if (j != 0) {
          for (unsigned idx=0; idx<ItersToPreserve.size(); idx++) {
            if (ItersToPreserve[idx]==i) {
              NeedConditional = true;
            }
          }
        }
        //errs() << "Iteration " << i << " needs conditional? " << NeedConditional << "\n";
      }
      else {
    ...
    if (NeedConditional) {
      ...
      BranchInst* NewBR = BranchInst::Create(Dest, Term);
      //errs() << "Replacing " << *Term << " with " << *NewBR << "\n";
